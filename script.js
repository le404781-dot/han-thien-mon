...
async function checkSession(){
 ...
 try{const d=await api('/api/me',{headers:authHeaders()});accountUI(d.user);await loadProfile();await loadChat();await loadLeaderboard();await loadTreasure();await loadMarket();await loadQuests();}
...
function renderProfile(p){
 ...
 <div class="inventory-panel attribute-panel"><div class="attribute-head"><span class="eyebrow">🎒 VẬT PHẨM ĐỆ TỬ</span><h3>Kho vật phẩm trong Bảng thuộc tính</h3><small>Tất cả vật phẩm nhận được từ Tàng Bảo Các, Nhiệm Vụ Đường và Phường Thị đều hiển thị tại đây.</small></div><div class="inventory-grid">${(p.inventory||[]).length?(p.inventory||[]).map(i=>`<article class="inventory-card"><span class="item-seal">${i.category==='Đan dược'?'◈':'⚔'}</span><div><span class="eyebrow">${esc(i.category)} · ${esc(i.grade||'Phàm phẩm')}</span><h3>${esc(i.name)} <small>×${Number(i.quantity||0)}</small></h3><p><b>Công dụng:</b> ${esc(i.description)}${Number(i.spirit_gain||0)>0?` · +${Number(i.spirit_gain).toLocaleString('vi-VN')} linh lực/lần dùng`:''}</p><p><b>Phẩm cấp:</b> ${esc(i.grade||'Phàm phẩm')}</p><p><b>Số lượng:</b> ${Number(i.quantity||0).toLocaleString('vi-VN')}</p></div></article>`).join(''):`<div class="empty-state compact"><h3>Chưa có vật phẩm</h3><p>Vật phẩm sau khi mua hoặc nhận thưởng sẽ tự động xuất hiện tại đây.</p></div>`}</div></div>`;
...
async function loadTreasure(){
 try{
  ...
  area.innerHTML=`<div class="treasure-wallet"><span>☯ Linh lực hiện có</span><strong>${Number(d.spiritPower).toLocaleString('vi-VN')}</strong><small>${esc(d.realm)} · Tầng ${d.tier}/9 · Vật phẩm mua thành công sẽ chuyển thẳng vào Bảng thuộc tính</small></div>
  ...
  document.querySelectorAll('.buy-item').forEach(b=>b.onclick=async()=>{
    ...
    try{
      ...
      $('#treasureMsg').textContent='Vật Phẩm đã được chuyển về bảng thuộc tính - mở bảng để xem';
      await Promise.all([loadProfile(),loadTreasure()]);
