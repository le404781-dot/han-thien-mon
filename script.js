...
const avatarHtml=(v,cls='')=>{const x=String(v??'').trim();if(/^data:image\//i.test(x))return `<img class="avatar-img ${cls}" src="${esc(x)}" alt="Ảnh đại diện" loading="lazy">`;return esc(x||'🧑🏻‍🎓');};
...
async function loadDisciples(){
 ...
 try{
  ...
  ${inviteOut.length?`<div class="friend-subtitle">⌛ LỜI MỜI NHẬP MÔN ĐÃ GỬI</div><div class="friend-list">${inviteOut.map(x=>`<article class="friend-card"><span class="friend-avatar">${avatarHtml(x.avatar)}</span><div><b>${esc(x.display_name)}</b><small>Đang chờ môn nhân chấp thuận</small></div></article>`).join('')}</div>`:''}
  ...
  ${discipleCandidates.length?`<div class="friend-subtitle">📜 MỜI NHẬP MÔN · CẢNH GIỚI THẤP HƠN</div><div class="friend-list">${discipleCandidates.map(x=>`<article class="friend-card"><span class="friend-avatar">${avatarHtml(x.avatar)}</span><div><b>${esc(x.display_name)}</b><small>${esc(x.rank||'Đệ tử')} · ${Number(x.spirit_power||0).toLocaleString('vi-VN')} linh lực</small></div><button class="btn small primary disciple-invite" data-id="${x.id}">📜 Mời nhập môn</button></article>`).join('')}</div>`:''}
...
async function loadFriends(){
 ...
 try{
  ...
  ${incoming.length?`<div class="friend-subtitle">📨 Lời mời đến</div><div class="friend-list">${incoming.map(x=>`<article class="friend-card"><span class="friend-avatar">${avatarHtml(x.avatar)}</span><div><b>${esc(x.display_name)}</b><small>${esc(x.rank||x.title||'Đệ tử')}</small></div><button class="btn small primary friend-accept" data-id="${x.id}">Chấp nhận</button><button class="btn small ghost friend-reject" data-id="${x.id}">Từ chối</button></article>`).join('')}</div>`:''}
  ${outgoing.length?`<div class="friend-subtitle">⌛ Đã gửi</div><div class="friend-list">${outgoing.map(x=>`<article class="friend-card"><span class="friend-avatar">${avatarHtml(x.avatar)}</span><div><b>${esc(x.display_name)}</b><small>Đang chờ chấp nhận</small></div></article>`).join('')}</div>`:''}
  <div class="friend-subtitle">☯ Danh sách bằng hữu</div><div class="friend-list">${friends.length?friends.map(x=>`<article class="friend-card"><span class="friend-avatar">${avatarHtml(x.avatar)}</span><div><b>${esc(x.display_name)}</b><small>${esc(x.rank||x.title||'Đệ tử')} · ${Number(x.spirit_power||0).toLocaleString('vi-VN')} linh lực</small></div><button class="btn small primary friend-chat" data-id="${x.id}" data-name="${esc(x.display_name)}">💬 Chat</button><button class="btn small ghost friend-remove" data-id="${x.id}">Hủy bạn</button></article>`).join(''):`<div class="empty-state compact"><p>Chưa có bằng hữu. Hãy mở Danh sách môn nhân để kết giao.</p></div>`}</div><p id="friendsMsg" class="train-msg"></p>`;
...
async function openFriendChat(userId,name){
 ...
 modal.showModal();
 const render=rows=>{const w=$('#privateChatWindow');w.innerHTML=rows.length?rows.map(x=>`<article class="chat-msg ${Number(x.sender_id)===Number(currentUser?.id)?'mine':''}"><span class="chat-avatar">${avatarHtml(x.avatar)}</span><div><div class="chat-meta"><b>${esc(x.display_name)}</b><time>${new Date(x.created_at).toLocaleTimeString('vi-VN',{hour:'2-digit',minute:'2-digit'})}</time></div><p>${esc(x.message)}</p></div></article>`).join(''):`<div class="chat-empty">Hai người chưa có cuộc trò chuyện nào.</div>`;w.scrollTop=w.scrollHeight;};
...
async function loadProfile(){
 try{const d=await api('/api/profile',{headers:authHeaders()});currentProfile=d.profile;renderProfile(currentProfile);renderCultivation(currentProfile);loadFriends();loadDisciples();loadChallenges();loadCodex();loadTienPhap();loadSpiritRankings();loadMansion();if(Number(currentProfile.trainCount||0)>=Number(currentProfile.maxDaily||10) && !Boolean(currentProfile?.mansion?.active))startOnlineCultivation();else if(onlineTimer){clearInterval(onlineTimer);onlineTimer=null;}loadAchievements();loadTreasure();loadDanCac();loadBeastHouse();loadLinhPhap();loadEquipment();loadBicanh();loadProfessions();loadQuests();renderLegendEditor();}
...
async function onlineCultivationTick(){
  ...
  try{
    ...
    if(d.mode==='online'){
      if(status)status.textContent=`🟢 Đang tích lũy Online · +${Number(d.rate||0).toLocaleString('vi-VN')} linh lực/phút · khoảng ${Number(d.ratePerHour||0).toLocaleString('vi-VN')}/giờ`;
    ...
    if(status && Number(currentProfile?.trainCount||0)<Number(currentProfile?.maxDaily||10)) status.textContent='🔒 Chưa mở Online · Hoàn thành đủ lượt vận công hôm nay';
  }
}
...
function renderCultivation(p){
 ...
 const onlineUnlocked=trainCount>=maxDaily && !mansionActive;
 ...
 <div class="online-cultivation-card ${onlineUnlocked?'online-ready':'online-locked'}" id="onlineCultivationPanel"><div><span class="eyebrow">☁ TU LUYỆN ONLINE</span><h3>${onlineUnlocked?'Tự động tích lũy linh lực':'Chưa mở Online'}</h3><p id="onlineStatus">${mansionActive?`🏯 ${esc(p.mansion.name)} đang khởi động · Online bị khóa tương tự vận công.`:onlineUnlocked?`🟢 +${onlineRate.toLocaleString('vi-VN')} linh lực/phút · khoảng ${(onlineRate*60).toLocaleString('vi-VN')} linh lực/giờ · cảnh giới ${esc(p.realm)} càng cao tốc độ càng giảm.`:`🔒 Hoàn thành đủ ${maxDaily}/${maxDaily} lượt vận công để mở Online.`}</p></div><div class="online-numbers"><b id="onlineGain">${onlineUnlocked?'Tích lũy theo thời gian · giới hạn 600/ngày':'Đang chờ đủ lượt vận công'}</b><small>Không tiêu hao thêm lượt vận công</small></div></div><p id="trainMsg" class="train-msg">${mansionActive?`🏯 ${esc(p.mansion.name)} đang khởi động: tự động +${Number(p.mansion.spiritPerHour||0).toLocaleString('vi-VN')} linh lực/giờ. Vận công bị khóa hoàn toàn.`:`Lượt tu luyện hôm nay = lượt vận công: <b>${trainCount}/${maxDaily}</b>. ${trainCount>=maxDaily?'Đã hết lượt vận công hôm nay; Online đã mở.':'Khi hết lượt, nút Vận công sẽ tự khóa; Online sẽ mở tự động.'}`}</p>`;
...
function accountSummary(){
 ...
 $('#accountContent').innerHTML=`<div class="account-box"><div class="avatar-mini">${avatarHtml(currentProfile.avatar)}</div><span class="eyebrow">ĐÃ NHẬP MÔN</span><h3>${esc(currentProfile.display_name)}</h3><p>${esc(currentProfile.stage||currentProfile.rank)} · ${esc(currentProfile.title)}</p><p>Linh lực: <b>${Number(currentProfile.spirit_power).toLocaleString('vi-VN')}</b></p><p>Linh thạch: <b>💎 ${Number(currentProfile.spirit_stones||0).toLocaleString('vi-VN')}</b></p><div class="account-actions"><button id="profileEditQuick" class="btn primary">Hồ sơ đệ tử</button><button id="logoutBtn" class="btn ghost">Rời phiên</button></div></div>`;
