...
async function loadEquipment(){
 ...
 try{
  ...
  const beastCards=(d.beasts||[]).map(x=>`<article class="equipment-item item-avatar-picker"><div class="equipment-item-head"><div class="item-avatar">${avatarHtml(x.avatar)}</div><div><span class="eyebrow">🐉 ${esc(x.rarity)} · ${esc(x.beast_realm)} ${x.beast_realm_tier}</span></div></div>${currentProfile?.item_avatar_unlocked?`<label class="item-avatar-upload">🖼 Đổi ảnh<input class="item-avatar-file" type="file" accept="image/png,image/jpeg,image/webp,image/gif" data-type="beast" data-id="${x.beast_id}"></label>`:`<small class="item-avatar-locked">🔒 Ảnh đại diện vật phẩm chưa mở</small>`}<h3>${esc(x.name)} ×${x.quantity}</h3><p>⚔ ${x.attack} · 🛡 ${x.defense} · 💨 ${x.speed} · ☯ ${x.spirit}</p><span class="equip-meta">Chiến lực +${Number(x.power_bonus||0).toLocaleString('vi-VN')}</span><p>${esc(x.ability||x.skill||'—')}</p><div class="equipment-actions"><button class="btn small primary" data-equip-type="beast" data-equip-id="${x.beast_id}">Trang bị</button></div></article>`).join('');
  ...
  const artifactCards=(d.artifacts||[]).map(x=>`<article class="equipment-item item-avatar-picker"><div class="equipment-item-head"><div class="item-avatar">${avatarHtml(x.avatar)}</div><div><span class="eyebrow">⚔ ${esc(x.category)} · ×${x.quantity}</span></div></div>${currentProfile?.item_avatar_unlocked?`<label class="item-avatar-upload">🖼 Đổi ảnh<input class="item-avatar-file" type="file" accept="image/png,image/jpeg,image/webp,image/gif" data-type="artifact" data-id="${x.id}"></label>`:`<small class="item-avatar-locked">🔒 Ảnh đại diện vật phẩm chưa mở</small>`}<h3>${esc(x.name)}</h3><p>${esc(x.description)}</p><span class="equip-meta">Chiến lực +${Number(x.power_bonus||0).toLocaleString('vi-VN')}</span><p>${esc(x.ability||'Không có công năng riêng.')}</p><div class="equipment-actions"><button class="btn small primary" data-equip-type="artifact" data-equip-id="${x.id}">Trang bị</button></div></article>`).join('');
