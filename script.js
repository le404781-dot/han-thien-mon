...
async function loadBicanh(){
 ...
 try{
  ...
  const selector=realms.map(r=>`<option value="${Number(r.required_realm_index)}" ${Number(r.required_realm_index)===currentIndex?'selected':''}>${esc(r.required_realm_name)} — ${esc(r.name)}${Number(r.required_realm_index)!==currentIndex?' 🔒':''}</option>`).join('');
  ...
  const cards=realms.map((r,i)=>{
   const matching=Number(r.required_realm_index)===currentIndex;
   ...
   let action='';
   if(locked) action=`<button class="btn small" disabled>🔒 Chỉ ${esc(r.required_realm_name)}</button>`;
   ...
   return `<article class="bicanh-card ${locked?'locked':''} ${active?'active':''}" data-realm-index="${Number(r.required_realm_index)}">
    ...
    <div class="bicanh-meta"><span>☠ Nguy hiểm <b>${Number(r.danger_percent)}%</b></span><span>☠ Debuff <b>${Number(r.debuff_percent)}%</b></span><span>🎁 Cấp thưởng <b>${Number(r.loot_tier)+1}</b></span><span>👥 Tham gia <b>${count}/2+</b></span></div>
    <div class="bicanh-party"><div class="friend-subtitle">👥 Đội thám hiểm</div>${participantNames||'<small>Chưa có môn nhân nào nhấn tham gia.</small>'}<p><b>Luật:</b> phải có tối thiểu 2 môn nhân cùng đúng cảnh giới mới được tiến vào.</p></div>
    ${matching?`<div class="bicanh-invite"><select class="bicanh-friend-select" data-id="${r.id}">${friendOptions()}</select><button class="btn small primary bicanh-invite-btn" data-id="${r.id}">📨 Mời Bằng Hữu</button></div>`:''}
