...
async function loadProfile(){
 try{const d=await api('/api/profile',{headers:authHeaders()});currentProfile=d.profile;renderProfile(currentProfile);renderCultivation(currentProfile);loadFriends();loadDisciples();loadChallenges();loadCodex();loadSpiritRankings();loadMansion();if(Number(currentProfile.trainCount||0)>=Number(currentProfile.maxDaily||10) && !Boolean(currentProfile?.mansion?.active))startOnlineCultivation();else if(onlineTimer){clearInterval(onlineTimer);onlineTimer=null;}loadAchievements();loadTreasure();loadBeastHouse();loadLinhPhap();loadEquipment();loadBicanh();loadProfessions();loadQuests();renderLegendEditor();}
...
async function loadBicanh(){
 ...
 try{
  ...
  const inviteBox=invites.length?`<div class="bicanh-invites"><div class="friend-subtitle">📨 LỜI MỜI BÍ CẢNH</div>${invites.map(x=>`<div class="friend-card"><span class="friend-avatar">${esc(x.inviter_avatar||'🧑🏻‍🎓')}</span><div><b>${esc(x.inviter_name)}</b><small>Mời bạn vào ${esc(x.realm_name)} · ${esc(x.required_realm_name)}</small></div><button class="btn small primary bicanh-invite-accept" data-id="${x.id}">Tham gia</button><button class="btn small ghost bicanh-invite-reject" data-id="${x.id}">Từ chối</button></div>`).join('')}</div>`:'';
...
function renderChat(rows){
 $('#chatArea').innerHTML=`<div class="chat-window" id="chatWindow">${rows.length?rows.map(x=>{const arrival=x.isElder&&x.message==='Đại Lão Đã Đến ⛩️';return arrival?`<article class="elder-arrival"><span>⛩️</span><b>Đại Lão Đã Đến ⛩️</b><small>${esc(x.display_name)} đã bước vào Chat Tổng.</small></article>`:`<article class="chat-msg ${Number(x.user_id)===Number(currentProfile?.user_id)?'mine':''}"><span class="chat-avatar">${esc(x.avatar||'🧑🏻‍🎓')}</span><div><div class="chat-meta"><b>${esc(x.display_name)}</b>${x.isElder?'<span class="elder-badge">👑 Đại Lão</span>':''}<span>${esc(x.rank)}</span><time>${new Date(x.created_at).toLocaleTimeString('vi-VN',{hour:'2-digit',minute:'2-digit'})}</time></div><p>${esc(x.message)}</p></div></article>`;}).join(''):`<div class="chat-empty">Sơn môn còn tĩnh lặng. Hãy gửi lời chào đầu tiên.</div>`}</div><form id="chatForm" class="chat-form"><input id="chatInput" maxlength="500" autocomplete="off" placeholder="Truyền âm tới toàn môn..." required><button class="btn primary">Gửi</button></form><p id="chatMsg" class="train-msg"></p>`;
