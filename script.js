...
function renderXuatQuan(){
 ...
 grid.innerHTML=online.length?online.map(m=>`<article class="member-card"><div class="avatar">${avatarHtml(m.emoji,'',m.realmIndex)}</div><div class="member-info"><span class="nickname">${esc(m.nick||'')}</span><h3>${esc(m.name)}</h3><p>${esc(m.role||'Đệ tử')}</p><div class="presence-status online"><span class="presence-dot"></span>Đang xuất quan</div><div class="tags">${(Array.isArray(m.tags)?m.tags:[]).map(t=>`<span class="tag">${esc(t)}</span>`).join('')}</div>${m.equipped_beast_name?`<div class="member-beast-badge">🐉 ${esc(m.equipped_beast_name)} · ${esc(m.equipped_beast_realm||'Linh thú')} ${Number(m.equipped_beast_realm_tier||1)} · +${Number(m.equipped_beast_gear_power||0)} trang bị</div>`:''}</div></article>`).join(''):`<div class="empty-state compact"><h3>Không có môn nhân đang xuất quan</h3><p>Hiện tại chưa có môn nhân nào đang hoạt động trên sơn môn.</p></div>`;
}
...
async function loadTreasure(){
 try{
  ...
  <div class="treasure-grid">${d.items.map(i=>{
    ...
    const use=owned>0 && Number(i.spirit_gain||0)>0 ? `<button class="btn small treasure-use-item" data-id="${i.id}">Dùng 1 · +${Number(i.spirit_gain).toLocaleString('vi-VN')} linh lực</button>` : '';
...
async function loadBeastArena(){
 ...
 try{
  const d=await api('/api/beast-arena',{headers:authHeaders()}); const beasts=d.beasts||[],members=d.members||[],npcs=d.npcs||[],pending=d.pending||[],history=d.history||[];
  const beastOptions=beasts.map(b=>`<option value="${b.beast_id}">${esc(b.name)} · ${esc(b.rarity)} · ${esc(b.beast_realm)}${Number(b.battle_debuff_percent||0)?` · ⚠️ -${b.battle_debuff_percent}%`:''}</option>`).join('');
  const npcCards=npcs.filter(n=>n.id!=='than-long').map(n=>`<article class="beast-arena-card ${n.id==='thien-ho'?'divine-card':''}"><div class="beast-arena-card-top"><span class="beast-arena-icon">${n.id==='thien-ho'?'🦊':'🪶'}</span><div><span class="eyebrow">${esc(n.title)}</span><h3>${esc(n.name)}</h3></div></div><p>${esc(n.rarity)} · ${esc(n.realm)} · ${esc(n.skill)}</p><div class="beast-arena-stats"><span>⚔ ${n.attack}</span><span>🛡 ${n.defense}</span><span>💨 ${n.speed}</span><span>☯ ${n.spirit}</span></div><button class="btn small ${n.id==='thien-ho'?'primary':''} beast-offline-btn" data-npc="${esc(n.id)}" ${beasts.length?'':'disabled'}>${n.id==='thien-ho'?'🔥 Thách Chiến Thần Thú':'⚔ Thách Chiến NPC'}</button></article>`).join('');
  const memberCards=members.map(m=>`<article class="beast-member-card"><div><b>${esc(m.display_name)}</b><small>${esc(m.rank||'Môn nhân')} · 🪶 ${Number(m.beast_count||0)} linh thú</small></div><button class="btn small primary beast-invite-btn" data-user="${m.id}" ${beasts.length?'':'disabled'}>🪶 Gửi lời mời</button></article>`).join('');
  ...
  const historyHtml=history.length?history.slice(0,12).map(x=>`<div class="beast-history-row"><span>${x.status==='completed'?'⚔️':'📬'}</span><div><b>${esc(x.challenger_name)} 🪶 ${esc(x.challenger_beast)} vs ${esc(x.opponent_name)} 🪶 ${esc(x.opponent_beast||'—')}</b><small>${x.winner_id?`🏆 ${Number(x.winner_id)===Number(d.me.userId)?'Bạn giành chiến thắng':'Đối thủ giành chiến thắng'}`:'Đang xử lý'} · ${x.reward_item?`🎁 ${esc(x.reward_item)} ×${Number(x.reward_quantity||1)}`:'Không có vật phẩm'} · ${Number(x.rounds||0)} hiệp</small></div></div>`).join(''):'<small class="muted">Chưa có chiến tích tại Thú Trường.</small>';
  area.innerHTML=`<div class="beast-arena-wallet"><div><span class="eyebrow">🪶 LINH THÚ XUẤT CHIẾN</span><h3>${beasts.length?esc(beasts[0].name):'Chưa có linh thú'}</h3><small>${beasts.length?'Chọn linh thú trước khi tiến vào chiến trường.':'Hãy nhận linh thú tại Thú Đường/Bí Cảnh trước.'}</small></div><div><b>🎒 ${beasts.length} linh thú có thể xuất chiến</b><select id="beastArenaSelected">${beastOptions||'<option value="0">Chưa có linh thú</option>'}</select></div></div>
  <div class="beast-arena-mode-grid"><article class="beast-mode-card"><span>⚔️</span><h3>Thách Chiến Online</h3><p>Gửi lời mời cho môn nhân khác. Trận đấu được mô phỏng theo chỉ số, tốc độ, linh lực và thiên phú của hai linh thú.</p><div class="beast-member-list">${memberCards||'<small class="muted">Chưa có môn nhân khác.</small>'}</div></article><article class="beast-mode-card"><span>🪶</span><h3>Thách Chiến Offline</h3><p>So tài với NPC để luyện chiến thuật. Thắng ngẫu nhiên nhận vật phẩm từ Dược Đường.</p><div class="beast-npc-grid">${npcCards}</div></article><article class="beast-mode-card divine-mode"><span>🔥</span><h3>Thách Chiến Thần Thú</h3><p>Độ khó cao nhất. Tỷ lệ và phẩm cấp vật phẩm Dược Đường được nâng mạnh. <b>Thất bại sẽ khiến linh thú nhận debuff ngẫu nhiên -35% trong 24 giờ.</b></p><div class="divine-warning">⚠️ Thần Thú phản phệ · Công / Thủ / Tốc / Thần ngẫu nhiên</div></article></div>
  <div class="beast-arena-lower"><div><h3>📬 Lời mời đang chờ</h3>${pendingHtml}</div><div><h3>📜 Chiến tích Thú Trường</h3><div class="beast-history-list">${historyHtml}</div></div></div><p id="beastArenaMsg" class="train-msg"></p>`;
  document.querySelectorAll('.beast-invite-btn').forEach(btn=>btn.onclick=async()=>{const beastId=Number($('#beastArenaSelected')?.value||0);if(!beastId)return;btn.disabled=true;try{const x=await api('/api/beast-arena/online/request',{method:'POST',headers:authHeaders(),body:JSON.stringify({targetUserId:Number(btn.dataset.user),beastId})});$('#beastArenaMsg').textContent='🪶 '+x.message;await loadBeastArena();await loadMailbox();}catch(e){$('#beastArenaMsg').textContent='❌ '+e.message;btn.disabled=false;}});
  document.querySelectorAll('.beast-offline-btn').forEach(btn=>btn.onclick=async()=>{const beastId=Number($('#beastArenaSelected')?.value||0);if(!beastId)return;btn.disabled=true;const divine=btn.dataset.npc==='thien-ho'||btn.dataset.npc==='than-long';try{const x=await api(divine?'/api/beast-arena/divine':'/api/beast-arena/offline',{method:'POST',headers:authHeaders(),body:JSON.stringify({beastId,npcId:btn.dataset.npc})});$('#beastArenaMsg').textContent=(x.win?'🏆 ':'💥 ')+x.message+(x.reward?` · 🎁 ${x.reward.name} ×${x.reward.quantity}`:'')+(x.debuff?` · ⚠️ ${x.debuff.text}`:'');await Promise.all([loadBeastArena(),loadTuDi(),loadProfile()]);}catch(e){$('#beastArenaMsg').textContent='❌ '+e.message;}finally{btn.disabled=false;}});
