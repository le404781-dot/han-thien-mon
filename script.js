...
async function loadTreasure(){
 try{
  ...
  <div class="stone-exchange"><div><span class="eyebrow">⚙ QUY TẮC TÀNG BẢO CÁC</span><h3>Linh lực → Bảo vật</h3><p>Giá vật phẩm được trừ trực tiếp vào linh lực. Linh thạch không còn dùng để mua vật phẩm.</p></div><span class="tag">☯ Dùng Linh lực</span></div>
  <div class="treasure-grid">${d.items.map(i=>{
    ...
    const buttonText=isLocked?'🔒 Cần '+esc(realmNames[Number(i.min_realm)]):canBuy?'☯ Mua bằng linh lực':'Thiếu '+Number(price-spirit).toLocaleString('vi-VN')+' linh lực';
